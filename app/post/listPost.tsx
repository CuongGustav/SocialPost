...
// Fetch all posts
const getAllPosts = async (): Promise<PostWithUserResponse[]> => {
  try {
...
const ListPost = () => {
  ...
  if (posts.length === 0) {
    return <div className="text-center">Không có bài đăng nào</div>;
  }
    ...
    <div className="flex flex-col">
      {posts.map((post) => (
        <div
          ...
          onClick={() => router.push(`/post/${post.id}`)} // Chuyển hướng đến /post/{post_id}
        >
          ...
          <div className="ml-2 mt-2 w-full">
            <div className="flex flex-row gap-2">
              <div className="font-bold max-w-[128px] overflow-hidden hover:underline">{post.username}</div>
              <div>
